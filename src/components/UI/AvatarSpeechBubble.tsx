  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={`absolute z-50 text-left bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl p-4 shadow-2xl border-2 border-teal-500/40 dark:border-teal-400/40 max-h-[85vh] overflow-y-auto ${
          isSidebar
            ? 'left-full top-0 ml-4 w-[20rem] sm:w-[23rem] lg:w-[25rem]'
      ...
      >
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <img
                ...
                className="w-7 h-7 rounded-full object-cover border-2 border-teal-500 shadow-sm"
              />
            ...
            <div>
              <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center gap-1">
                ...
                <span className="px-1.5 py-0.2 rounded-full text-[9px] font-semibold bg-teal-500/15 text-teal-700 dark:text-teal-300">
                  Live
                </span>
              </h3>
            </div>
        ...
        >
          <input
            ...
            placeholder="Ask about Jack's background..."
            aria-label="Ask AI Assistant a question"
            className="flex-1 px-3 py-1.5 rounded-lg bg-slate-100/90 dark:bg-slate-800/90 text-slate-900 dark:text-slate-100 text-[11px] font-medium placeholder-slate-400 border border-slate-200 dark:border-slate-700 focus:outline-none focus:border-teal-500"
          />
          <button
            type="submit"
          ...
          <div className="flex flex-wrap gap-1">
            {QUICK_PROMPTS.map((prompt, idx) => (
              <button
                ...
                className="text-left px-2 py-1 rounded-md text-[10px] font-medium bg-slate-100 dark:bg-slate-800/80 hover:bg-teal-500/10 dark:hover:bg-teal-500/20 hover:text-teal-700 dark:hover:text-teal-300 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700/80 transition-all cursor-pointer leading-tight"
              >
                {prompt}
              </button>
            ))}
        ...
        <div className="pt-2">
          {isTyping && (
            <div className="flex items-center gap-2 text-[11px] font-semibold text-teal-600 dark:text-teal-400 py-1.5">
          ...
          {!isTyping && currentResponse && (
            <motion.div
              ...
              className="space-y-2 p-3 rounded-xl bg-teal-500/10 dark:bg-teal-500/15 border border-teal-500/20 text-slate-800 dark:text-slate-200 text-xs leading-relaxed"
            >
